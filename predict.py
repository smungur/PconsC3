...
def parsePSSM(alignment):
    ...
    bi = [0.0825, 0.0553, 0.0406, 0.0545, 0.0137, 0.0393, 0.0675, 0.0707, 0.0227,
          ...
          0.0292, 0.0687]
    b = {aa: bi[i] for i, aa in enumerate(one2number[:-1])}  # exclude gap
    freqs = {}
    seqcount = 0.
    gapcount = 0
    coverage = []
    ...
    for l in open(alignment):
        ...
        for i in range(len(x)):
            freqs.setdefault(i, {}).setdefault(x[i], 0)
            freqs[i][x[i]] += 1
            if x[i] == '-':
                gapcount += 1

    if seqcount == 0 or len(freqs) == 0:
        raise ValueError("Empty or invalid alignment.")

    b['-'] = gapcount / (seqcount * len(freqs))

    entropy = []
    for i in sorted(freqs.keys()):
        q = []
        for aa in one2number:
            denom = b.get(aa, 1e-6) * seqcount
            if denom == 0:
                denom = 1e-6  # avoid division by zero
            freq = freqs[i].get(aa, 0)
...
def predict(dir, X):
    ...
    for t in trees:
        ...
        for i in range(len(X)):
            ...
            predictions[i] += rrr
        count += 1
        sys.stderr.write('\rProgress: [' + '#' * int(80*count/allcount) + ' ' * int(80 - (80*count)/allcount) + ']')
